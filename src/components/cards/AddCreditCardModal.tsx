...
import { CreditCard, CardBrand, CardIssuerId } from '../../types/finance';
...
const ISSUERS: { id: CardIssuerId; label: string; defaultBank: string }[] = [
  { id: 'generic', label: 'Genérico / Estándar', defaultBank: 'Banco' },
  { id: 'nu', label: 'Nu Colombia', defaultBank: 'Nu Colombia' },
  { id: 'bancolombia', label: 'Bancolombia', defaultBank: 'Bancolombia' },
  { id: 'rappicard', label: 'RappiCard', defaultBank: 'RappiCard' },
];
...
export const AddCreditCardModal: React.FC<AddCreditCardModalProps> = ({
  ...
  const [issuerId, setIssuerId] = useState<CardIssuerId>('generic');
  ...
  useEffect(() => {
    if (cardToEdit) {
      ...
      setIssuerId(cardToEdit.issuerId || 'generic');
  ...
  const handleIssuerSelect = (issuer: typeof ISSUERS[0]) => {
    setIssuerId(issuer.id);
    if (!bankName.trim() || bankName === 'Banco' || ISSUERS.some((i) => i.defaultBank === bankName)) {
      setBankName(issuer.defaultBank);
    }
    if (!name.trim()) {
      setName(issuer.label);
    }
  };
      ...
      <View style={styles.modalOverlay}>
        ...
        <View style={styles.modalContainer}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
            {/* Emisor / Políticas Bancarias */}
            <Text style={styles.label}>Emisor Bancario (Reglas de Imputación de Pagos)</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.brandsRow}>
              {ISSUERS.map((iss) => {
                const isSelected = issuerId === iss.id;
                return (
                  <TouchableOpacity
                    key={iss.id}
                    style={[styles.brandChip, isSelected && styles.brandChipSelected]}
                    onPress={() => handleIssuerSelect(iss)}
                  >
