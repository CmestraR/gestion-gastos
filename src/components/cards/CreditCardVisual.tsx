        ...
        <View style={styles.datesGrid}>
          ...
          <View style={styles.dateBox}>
            <View style={styles.dateIconRow}>
              <CustomIcon name="Calendar" size={13} color="#CBD5E1" />
              <Text style={styles.dateTitle}>Día de Corte: {card.cutOffDay}</Text>
            </View>
            <Text style={styles.dateSubtitle}>
              {daysToCutOff === 0 ? '¡Corte hoy!' : daysToCutOff > 0 ? `Cierra en ${daysToCutOff} d` : 'Cerrado'}
            </Text>
          </View>

          {/* Fecha Límite de Pago */}
          <View style={[styles.dateBox, daysToPayment <= 5 && styles.dateBoxUrgent]}>
            <View style={styles.dateIconRow}>
              <CustomIcon
                name="AlertCircle"
                size={13}
                color={daysToPayment <= 5 ? '#F87171' : '#CBD5E1'}
              />
              <Text style={[styles.dateTitle, daysToPayment <= 5 && styles.dateTitleUrgent]}>
                Día de Pago: {card.paymentDueDay}
              </Text>
            </View>
            <Text style={[styles.dateSubtitle, daysToPayment <= 5 && styles.dateSubtitleUrgent]}>
              {daysToPayment < 0 ? 'Vencido' : daysToPayment === 0 ? '¡Paga Hoy!' : `Vence en ${daysToPayment} d`}
            </Text>
          </View>
        </View>
