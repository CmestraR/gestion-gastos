...
export const AccountRepository = {
  ...
  async delete(id: string): Promise<void> {
    ...
    await db.withTransactionAsync(async () => {
      // 1. Verificar si la cuenta tiene historial de transacciones
      ...
      const hasHistory = (txCount?.count || 0) > 0;

      if (hasHistory) {
        // POLÍTICA: Si tiene historial financiero, ARCHIVAR para preservar integridad contable
