...
export function calculateCardCycleDates(
  ...
  if (paymentDueDay <= cutOffDay) {
    // Escenario estándar: Corte fin de mes (ej. día 28), Pago mes siguiente (ej. día 10)
    // El ciclo que va corriendo actualmente:
    // Si hoy es <= paymentDueDay (ej. 1 al 10 del mes), el ciclo que se vence para pago es el que cortó el mes anterior
    if (currentDay <= paymentDueDay) {
      cutOffDate = new Date(year, month - 1, cutOffDay);
      paymentDueDate = new Date(year, month, paymentDueDay);
    } else {
      // Si hoy es > paymentDueDay (ej. 11 al 31 del mes), el corte a evaluar es el de este mes
      // y la fecha límite de pago es el mes siguiente
      cutOffDate = new Date(year, month, cutOffDay);
      paymentDueDate = new Date(year, month + 1, paymentDueDay);
    }
  } else {
    // Escenario: Corte y Pago ocurren en el mismo mes (ej. Corte día 5, Pago día 20)
    if (currentDay <= paymentDueDay) {
      cutOffDate = new Date(year, month, cutOffDay);
      paymentDueDate = new Date(year, month, paymentDueDay);
    } else {
      // Ya pasó la fecha de pago de este mes, pasamos al siguiente ciclo
      cutOffDate = new Date(year, month + 1, cutOffDay);
      paymentDueDate = new Date(year, month + 1, paymentDueDay);
    }
  }
