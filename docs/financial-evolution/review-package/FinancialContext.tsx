...
export const FinancialProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const payCardInstallment = async (
    ...
    // Ejecuta de forma 100% atómica la liberación de cupo (1 vez), cobro en cuenta (1 vez) y registro de transacción
    await CardRepository.payInstallmentAtomic(
      installmentId,
      purchaseId,
      principalAmount,
      totalAmount,
  ...
  const payCreditCard = async (cardId: string, amount: number, accountId: string, notes?: string) => {
    ...
    // TransactionRepository.create es la ÚNICA fuente autorizada para debitar cuenta y liberar cupo
    await TransactionRepository.create(tx);
    await loadData();
  };
